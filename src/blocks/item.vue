		...
		<div class="item" @dblclick="open">
			...
			<div class="pwContent">
				<div v-if="settings['item-tagline'] !== false" class="pwTagline">
					...
					<span v-else class="placeholder">{{ $t('kirbyblock-monstercards.item.tagline.placeholder') }}</span>
				</div>

				<div v-if="settings['item-heading'] !== false" class="pwHeading">
					<div v-if="parsedHeading">{{ parsedHeading }}</div>
					<div v-else class="placeholder">
						{{ $t('kirbyblock-monstercards.item.heading.placeholder') }}
					</div>
				</div>
				...
				<div v-if="settings['item-editor'] !== false" class="pwText">
					...
					<div v-else class="placeholder">
						{{ $t('kirbyblock-monstercards.item.description.placeholder') }}
					</div>
				</div>
			</div>
